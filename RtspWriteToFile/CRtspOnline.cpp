...
CRtspOnline::CRtspOnline(PCHAR input_url, FGetImageCallback f_get_image_callback)
	: CBaseRtsp(input_url)
{
	_f_get_image_callback = f_get_image_callback;
}

CRtspOnline::~CRtspOnline()
{
}

BOOL CRtspOnline::Start() {
	bool verbose = false;

	VSInput* input = open_input("rtsp", _input_url, verbose);
	VSOutput* output = open_output("rtp", "rtp://127.0.0.1:49990", input, verbose);

	char buf[200000];
	AVFormatContext *ac[] = { output->format_ctx };
	...
	fclose(fsdp);
	system("PAUSE");
	system("start "" D:\\VideoLAN\\VLC\\vlc.exe test.sdp");
	...
	while (true) {
		...
		if (write_packet2(input, output, packet, verbose) > 0) {
			//all ok;
		};

		if (write_packet(input, _f_get_image_callback, packet, verbose) > 0) {
			//all ok;
		};
...
int CRtspOnline::write_packet(const struct VSInput * const input, FGetImageCallback get_image_callback, AVPacket * const pkt, const bool verbose)
{
	if (!input || !get_image_callback || !pkt) {
		printf("%s\n", strerror(EINVAL));
		return -1;
	}
	...
	if (get_image_callback != nullptr) {
		get_image_callback(pkt->data, pkt->size);
	}
...
// -1 if error
// 1 if we wrote the packet
int CRtspOnline::write_packet2(const struct VSInput * const input, struct VSOutput * const output, AVPacket * const pkt, const bool verbose)
{
	if (!input || !output || !pkt) {
		printf("%s\n", strerror(EINVAL));
		return -1;
